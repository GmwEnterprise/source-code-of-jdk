...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        int tableLen, tableIndex;

        // 哈希表为空时初始化哈希表
        if (table == null || (tableLen = table.length) == 0)
            tableLen = (table = resize()).length;

        // 定位该key在hash表中的位置
        tableIndex = (tableLen - 1) & hash;
        Node<K, V> p = table[tableIndex];
        if (p == null) {
            // 该链头节点为空则赋值，方法结束
        ...
        ++modCount; // 操作数自增
        ++size;
        if (size > threshold) // size自增后计算是否大于阈值
            resize(); // 若大于阈值则resize
